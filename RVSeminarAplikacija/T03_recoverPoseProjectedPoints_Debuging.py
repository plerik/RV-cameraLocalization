...
# --- the projection to image frames doesn't seem to work as I think it should
# and I was unsuccessful to find what is wrong it the time i have availible
# to finish the project. (I should return to look on that when I have time)
...
for k in range(N):
    ...
    # Camera 2 position
    x = k/N*100
    alpha = -np.arctan(x/40) #-np.pi/30*k
    rvec2 = np.array([[0., alpha, 0]])
    # rvec2 = np.array([[0., 0, 0]])
    # tvec2 = np.array([[0., 0, 20*np.sin(2*np.pi*k/15)]]).T
    tvec2 = np.array([[x, 0, 0.]]).T
    # tvec2 = np.array([[0., 0, -40]]).T
    # # print(rotMat(alpha, 1))
