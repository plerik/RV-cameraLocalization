import numpy as np
import cv2
import glob

# ***
# Calibrates the camera (gets camera parameters) from the photos in the specified folder
#
# ***

cameraName = 'EriksPhoneCam'
folderName = 'CameraCalibration/'
folderPath = folderName + cameraName + '/'


# Chess caliber dimensions ()
calibNumW = 8
calibNumH = 6
calibSquareSize = 1


# termination criteria
criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)

# prepare object points, like (0,0,0), (1,0,0), (2,0,0) ....,(6,5,0)
objp = np.zeros((calibNumW*calibNumH,3), np.float32)
objp[:,:2] = calibSquareSize*np.mgrid[0:calibNumW,0:calibNumH].T.reshape(-1,2)

# Arrays to store object points and image points from all the images.
objpoints = [] # 3d point in real world space
imgpoints = [] # 2d points in image plane.



images = glob.glob(folderPath + '*.jpg')
print(f'Found {len(images)} images.')



for fname in images:
    img = cv2.imread(fname)
    gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)

    # Find the chess board corners
    ret, corners = cv2.findChessboardCorners(gray, (calibNumW,calibNumH), None)

    # If found, add object points, image points (after refining them)
    if ret == True:
        objpoints.append(objp)

        corners2 = cv2.cornerSubPix(gray,corners,(11,11),(-1,-1),criteria)
        imgpoints.append(corners2)

        # Draw and display the corners
        img = cv2.drawChessboardCorners(img, (calibNumW,calibNumH), corners2,ret)
        cv2.imshow('img',img)
        cv2.waitKey(1)
        
cv2.destroyAllWindows()

# Calibration: 
print('Starting the calibration proces...')
ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(objpoints, imgpoints, gray.shape[::-1],None,None)

print('Calibration done! \nThe parameters are:')
print(ret, mtx, dist, rvecs, tvecs)



# Save camera calibration to file:
cameraParametersPath = folderName + cameraName + '.npz'
np.savez(cameraParametersPath, retVal=ret, cameraMatrix=mtx, distCoeffs=dist, rvecs=rvecs, tvecs=rvecs)
print(f'Camera parameters saved to: {cameraParametersPath} ')

